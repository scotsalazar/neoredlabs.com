 ...
 *
 * Logos are displayed in grayscale by default and animate to full
 * colour on hover.  The grid is horizontally scrollable on small
 * screens and the logos are enlarged for greater prominence.
 */
const clients = [
          ...
          {clients.map((client) => (
            <motion.div key={client.name} className="flex-shrink-0">
              <img
                ...
                className="h-24 w-auto object-contain filter grayscale opacity-60 transition hover:grayscale-0 hover:opacity-100"
              />
            </motion.div>
          ))}
        </div>
